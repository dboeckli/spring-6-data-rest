...
class BeerWebControllerTest {
    ...
    void testEditBeer() {
        ...
        assertEquals(BEERFORM, editViewName);
        ...
        assertEquals(BEERFORM, beerFormViewName);

        String resultViewName = controller.updateBeer(beerId, beerToEdit);
        assertEquals(REDIRECT_PREFIX + LIST_BEERS_PATH, resultViewName);

        // Fetch the updated beer
        Model updatedModel = new ExtendedModelMap();
        controller.getBeer(beerId, updatedModel);
        Beer updatedBeer = (Beer) updatedModel.getAttribute("beer");
    ...
    void testCreateBeer() {
        ...
        assertEquals(REDIRECT_PREFIX + LIST_BEERS_PATH, listBeerViewName);

        List<Beer> beers = beerRepository.getByBeerName("new Beer");
